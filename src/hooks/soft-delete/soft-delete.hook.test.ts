...
async function setup(options: { type: 'before' | 'around' }) {
  ...
  const items = await userService.create([
    { name: 'Jane Doe', key: 'a' },
    { name: 'Jack Doe', key: 'a' },
  ...
  return { app, userService, items }
}

function testForHookType(type: 'before' | 'around') {
  let userService: any
  let users: any[]

  describe(`ctx.type === '${type}'`, () => {
    beforeEach(async () => {
      const result = await setup({ type })
    ...
    describe('update, with id', () => {
      ...
      // see https://github.com/feathersjs/feathers/pull/3617
      it.skip('throws on deleted item', async () => {
        const deletedUser = users.find((u) => !!u.deletedAt)
        if (!deletedUser) {
          throw new Error('No deleted user found')
        }
        await expect(
          userService.update(deletedUser.id, { y: 'y' }),
        ).rejects.toThrow()
      })
    })
