...
describe('passParams', () => {
  it('rule true includes the value as-is', () => {
    ...
    expect(passParams(params, { user: true })).toEqual({ user: { id: 5 } })
  })

  it('rule false drops the key', () => {
    const params = { query: { a: 1 }, paginate: false } as any
    expect(passParams(params, { query: true, paginate: false })).toEqual({
      query: { a: 1 },
    })
  })
  ...
  it('function returning true includes as-is', () => {
    ...
    expect(passParams(params, { query: () => true })).toEqual({
      query: { a: 1 },
    })
  })
  ...
  it('function returning false or undefined drops the key', () => {
    ...
    expect(passParams(params, { a: () => false, b: () => undefined })).toEqual(
      {},
    )
  })
  ...
  it('does not inject a top-level schema key absent from params', () => {
    const params = { query: {} } as any
    expect(passParams(params, { query: true, user: true })).toEqual({
      query: {},
    })
  })
  ...
  it('drops a nested key while keeping the parent via a projection', () => {
    ...
    expect(passParams(params, { query: (q) => _omit(q, '_$client') })).toEqual({
      query: { userId: 1 },
    })
  })
  ...
  it('keeps unknown keys except the ones the schema drops with false', () => {
    ...
    expect(passParams(params, { rateLimit: false })).toEqual({
      query: {},
      foo: 1,
    })
  })
  ...
  it('reports unknown keys once via onUnknownParams, excluding schema-false keys', () => {
    ...
    passParams(params, { query: true, rateLimit: false }, { onUnknownParams })
  ...
  it('does not mutate params or nested objects', () => {
    ...
    passParams(params, { query: true, foo: false })
    passParams(params, { 'query.a': true })
